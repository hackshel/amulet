...
static void smaptotable(lua_State *lua, struct SMAP *map){
	...
	for(p = smap_get_first(map, &pair, keybuf, KEYTYPE_ALL, 0);
		p != NULL;
		(p = smap_get_next(map, &pair, keybuf, KEYTYPE_ALL, 0)) != NULL) {
			vbuf = (char *)SMAP_GET_VALUE(p);
			printf("%s: key: %s, value: %s\n", __func__, SMAP_GET_STR_KEY(p), vbuf);
			lua_pushlstring(lua, vbuf, SMAP_GET_VALUE_LEN(p));
...
//	lua_getfield(lua, 1, TDBDATAVAR);
	...
	for (p = smap_get_first(map, &pair, keybuf, KEYTYPE_ALL, 0);
		...
		if (c)
			printf("%s: key: %014ld, value: \"%s\"\n", __func__, (int64_t)p->ikey, (char *)p->data);
		else
			printf("%s: key: \"%s\", value: \"%s\"\n", __func__, (char *)p->skey, (char *)p->data);
...
main(void)
{
	...
	SMAP_SET_STR_PAIR(&pair, "a", 1, "test111", 8);
	smap_put(map, &pair, 1);
	SMAP_SET_STR_PAIR(&pair, "b", 1, "test222", 8);
	smap_put(map, &pair, 1);
